...
if sysre:=re.search(r'^(.+)\.vm$',sys.argv[1]):
    name=sysre.group(1)
n=0  
with open (f"{sys.argv[1]}","r") as file:
    for line in file:
        ...
        if Nott:=re.search(r'^not *\/?\/?',line):
            ...
            with open(f"{name}.asm","a") as asm:
                    asm.write (f"{ans}")
        if labell:=re.search(r'^label ([^\/]+) *\/?\/?',line):
        ...
        if gotoo_un:=re.search(r'^goto (.+) *\/?\/?',line):
            ...
            with open(f"{name}.asm","a") as asm:
                asm.write (f"{ans}")
            n+=1
        if funccc:=re.search(r'^function ([^\/]+) ([0-9]+) *\/?\/?',line):
        ...
        if ret:=re.search(r'^return *\/?\/?',line):
            ...
            with open(f"{name}.asm","a") as asm:
                asm.write (f"{ans}")
        if call_fu:=re.search(r'^call ([^\/]+) ([0-9]+) *\/?\/?',line):
            ans=call_func(call_fu.group(1),call_fu.group(2),n)
            with open(f"{name}.asm","a") as asm:
                asm.write(f"{ans}")
                print(call_fu.group(1))
            n+=1
